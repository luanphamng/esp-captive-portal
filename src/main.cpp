...
void setUpWebserver(AsyncWebServer &server, const IPAddress &localIP) {
    ...
    server.on("/", HTTP_ANY, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse(LittleFS, webUiCaptivePortal, "text/html");
        response->addHeader("Cache-Control", "public,max-age=31536000");  // save this file to cache for 1 year (unless you refresh)
        request->send(response);
        Serial.println("Served Captive Portal Page");
    });

    // UI files
    server.serveStatic("/index.html", LittleFS, webUiHtml).setCacheControl("Cache-Control: public, max-age=604800");
